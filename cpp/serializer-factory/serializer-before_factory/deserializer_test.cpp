// compile command:
// gd98 deserializer_test.cpp ../shape/shape.cpp -I../shape/include -Wl,-rpath=. ../shape/lib/libglut_utils.so -L../shape/lib -lpoint
...
boost::shared_ptr<Shape> CircleCreator(std::map<std::string, std::string> m)
{
    ...
    boost::shared_ptr<Shape> circ(new Circle(radius, Point(pos_x, pos_y), angle, color));
    circ->Draw();
    return circ;
}
...
boost::shared_ptr<Shape> RectangleCreator(std::map<std::string, std::string> m)
{
    ...
    boost::shared_ptr<Shape> rec(new Rectangle(width, height, Point(pos_x, pos_y), angle, color));
    rec->Draw();
    return rec;
}
...
boost::shared_ptr<Shape> SquareCreator(std::map<std::string, std::string> m)
{
    ...
    boost::shared_ptr<Shape> sq(new Square(side, Point(pos_x, pos_y), angle, color));
    sq->Draw();
    return sq;
}
...
boost::shared_ptr<Shape> LineCreator(std::map<std::string, std::string> m)
{
    ...
    boost::shared_ptr<Shape> line(new Circle(length, Point(pos_x, pos_y), angle, color));
    line->Draw();
    return line;
}
