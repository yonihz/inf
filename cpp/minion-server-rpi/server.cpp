...
UDPRequestMgr::UDPRequestMgr(int sockfd_, Reactor *reactor_)
...
void UDPRequestMgr::operator()(void)
{
    Logger &logger = *(Singleton<Logger>::Instance());
    int status;
    ...
    status = sendto(m_sockfd, request_buffer, m_reply_len[(size_t)request_buffer[REQUEST_TYPE_BYTE]], MSG_WAITALL, &client_addrinfo, client_addrlen);
